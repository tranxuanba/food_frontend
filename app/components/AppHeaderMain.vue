    ...
    <div class="header-main">
      ...
      <v-menu open-on-hover location="bottom" offset="4" :open-on-click="false" :close-on-content-click="false">
        <template #activator="{ props }">
          <v-btn v-bind="props" color="#029d16" variant="flat" @click="shoppingCart">
        ...
        <v-card min-width="400">
          ...
          <div v-else>
                ...
                <v-col cols="7">
                  ...
                  <v-row class="ps-4 pt-1 d-flex align-center">
                    ...
                    <v-col cols="7" class="px-0"><v-number-input inset variant="solo-filled" v-model="item.quantity"
                        control-variant="split" elevation="0" class="no-shadow-number" hide-details
                        @update:model-value="(val) => onQuantityChange(item, val)"
                        density="compact"></v-number-input></v-col>
                  </v-row>
                  <v-row class="pb-1 ps-4">Giá: <span class="price">{{ formatPrice(item.price) }}</span></v-row>
                </v-col>
            ...
            <v-card-actions class="mb-2 d-flex justify-center">
              <v-btn class="btn-shopping-cart" text="Giỏ hàng" @click="handleShoppingCart" />
              <v-btn class="btn-payment-cart" text="Thanh toán" @click="handlePayment" />
            </v-card-actions>
          </div>
        </v-card>
      </v-menu>
...
const { setTotalCount, setCartItemMes, useCartItemMes, totalCount } = useCartItemMeList();
const { setFoods, setSearchFoodName } = useFoodList();
