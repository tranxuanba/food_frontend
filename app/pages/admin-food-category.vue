    ...
    <v-card elevation="0">
      ...
      <v-data-table :headers="headers" :items="categories" item-key="id" hide-default-footer>
        ...
        <template #item.actions="{ item }">
          <v-btn icon variant="text" color="red-lighten-1" @click="removeFoodInCart(item.categoryId)">
...
const addFoodCategoryApi = async () => {
  ...
  try {
    ...
    message.value = "Thêm sản phẩm vào giỏ hàng thành công";
    isSuccess.value = true;
  } catch (err) {
    message.value = "Lỗi thêm sản phẩm vào giỏ hàng";
...
const removeFoodInCart = async (categoryId: number) => {
...
const categoryNameRules = [
  ...
  (v: string | any[]) => (v && v.length <= 100) || 'Tên sản phẩm không được quá 100 ký tự'
]
</script>
